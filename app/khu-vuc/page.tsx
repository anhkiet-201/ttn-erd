        ...
        <main className="flex-1 ml-0 md:ml-20 lg:ml-72 p-4 md:p-8">
          <div className="flex items-center justify-between mb-8 max-w-4xl mx-auto">
            ...
            >
              ...
              <span className="font-semibold">Thêm Khu Vực</span>
            </button>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Tên Khu Vực</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  <tr>
                    <td colSpan={2} className="px-6 py-10 text-center text-gray-500">Đang tải dữ liệu...</td>
                  </tr>
                ) : filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={2} className="px-6 py-10 text-center text-gray-500">Không tìm thấy khu vực nào</td>
                  </tr>
                ) : (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="text-sm font-semibold text-gray-900">{item.tenKhuVuc}</div>
                      </td>
            ...
            {loading ? (
              ...
              filteredData.map((item) => (
                <div key={item.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                  <div className="mb-3">
                    <h3 className="font-semibold text-base text-gray-900 break-words">{item.tenKhuVuc}</h3>
                  </div>
